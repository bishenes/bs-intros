  ...
  async __loadDemo(name) {
    if (atom.docRepo) {
      ...
      if (Array.isArray(book.chapters)) {
        ...
        book = { libraryId: "books", id: uuidv4().split("-").join(""), name: book.displayName, cateName: book.cateName }
        ...
        let docOrderids = chapters.map(doc => {
          let content = fs.readFileSync(path.join(srcPath, doc.id + '.ot'))
          let chapter = atom.docRepo.newChapter(book, content);
          return chapter.id;
        })
        await atom.docRepo.dbProvider.updateDocsOrder(book.id, docOrderids)
      }
    }
  }
}